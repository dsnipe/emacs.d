...
(defun highlight-symbol-get-symbol ()
  ...
  (let ((symbol (thing-at-point 'symbol)))
    (when (and symbol (not (highlight-symbol-black-listed symbol)))
			(concat (car highlight-symbol-border-pattern)
                         (regexp-quote symbol)
                         (cdr highlight-symbol-border-pattern)))))

(defvar highlight-symbol-black-list '("end" "do" "module" "class" "if" "else" "gem" "def"))
(make-variable-buffer-local 'highlight-symbol-black-list)

(defun highlight-symbol-black-listed (symbol)
	"Check symbol in black list"
	(if highlight-symbol-black-list
		(member symbol highlight-symbol-black-list)))
